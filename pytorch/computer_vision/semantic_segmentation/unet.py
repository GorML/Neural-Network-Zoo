...
class UNet(Module):
    def __init__(self, in_channels=3, out_channels=2):
        ...
        self.b2 = Sequential(Conv2d(1024, 512, kernel_size=3), ReLU())

        # Decoder
        self.upconv1 = Sequential(ConvTranspose2d(512, 512, kernel_size=2, stride=2))
        self.dec1_1  = Sequential(Conv2d(1024, 512, kernel_size=3), ReLU())
        self.dec1_2  = Sequential(Conv2d(512, 256, kernel_size=3), ReLU())
        
        self.upconv2 = Sequential(ConvTranspose2d(256, 256, kernel_size=2, stride=2))
        self.dec2_1  = Sequential(Conv2d(512, 256, kernel_size=3), ReLU())
        self.dec2_2  = Sequential(Conv2d(256, 128, kernel_size=3), ReLU())
        
        self.upconv3 = Sequential(ConvTranspose2d(128, 128, kernel_size=2, stride=2))
        self.dec3_1  = Sequential(Conv2d(256, 128, kernel_size=3), ReLU())
        self.dec3_2  = Sequential(Conv2d(128, 64, kernel_size=3), ReLU())
        
        self.upconv4 = Sequential(ConvTranspose2d(64, 64, kernel_size=2, stride=2))
    ...
    def forward(self, x):
        ...
        d2 = self.dec2_2(self.dec2_1(torch.cat([e3_conv, self.upconv2(d1)], dim=1)))
        d3 = self.dec3_2(self.dec3_1(torch.cat([e2_conv, self.upconv3(d2)], dim=1)))
        d4 = self.dec4_2(self.dec4_1(torch.cat([e1_conv, self.upconv4(d3)], dim=1)))
        d_ = self.dec_fin(d4)
        return d_
